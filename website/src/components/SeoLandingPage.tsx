...
export default function SeoLandingPage(props: SeoLandingPageProps) {
  ...
  const translatedProVisual = copy.proVisual && locale === 'zh-CN' ? {
    ...
    steps: copy.proVisual.steps.map((step, index) => ({ ...step, title: (props.h1.includes('Tailwind') ? ['打开参考页面', '提取网页颜色', '整理项目调色板', '导出 Tailwind 配置'] : props.h1.includes('Design Token') ? ['打开参考页面', '提取界面颜色', '整理项目令牌', '导出 CSS 或 W3C 令牌'] : ['打开参考页面', '提取页面颜色', '整理多个项目', '建立可复用颜色令牌集', '导出更多格式'])[index] ?? '整理颜色', description: '选择需要的颜色并整理到当前项目中，为下一步导出做好准备。', alt: step.alt.replace(/^TokenTint|^Reference|^Dribbble/, 'TokenTint') }))
  } : copy.proVisual;
    ...
    <>
      ...
      <main className="seo-page">
        ...
        <section className="seo-section seo-muted"><div className="container">
          ...
          <ol className="workflow-list">{copy.workflow.map((step, index) => { const parts = step.split('. '); return <li key={step}><span className="workflow-kicker">0{index + 1} / {(locale === 'zh-CN' ? ['拾取', '整理', '导出', '交付'][index] : (copy.workflowKickers ?? ['Pick', 'Organize', 'Export'])[index])}</span>{locale === 'zh-CN' ? <span>{step}</span> : <><strong>{parts[0]}.</strong><span>{parts.slice(1).join('. ')}</span></>}</li>; })}</ol>
        </div></section>

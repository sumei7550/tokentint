...
function getChineseCopy(props: SeoLandingPageProps): SeoLandingPageProps {
  ...
  if (props.h1.includes('Chrome Color Picker')) {
    return { ...props, ...common, h1: 'Chrome Color Picker 与 Eyedropper 扩展', intro: '直接从网页拾取 HEX、RGB 或 HSL 颜色，查看最近颜色历史，并将需要的值保存到可复用的项目调色板。', problem: '网页取色时，你可能会反复拾取同一区域、把复制的值丢在笔记里，或手动切换不同格式。基础取色器只能给你一个颜色，却无法帮助你查看周围的颜色并整理使用。', solution: 'TokenTint 让网页取色保持简单：选择颜色，在 HEX、RGB 和 HSL 之间切换，查看最近历史，并将有用的值保存到项目调色板中。', workflowKickers: ['拾取', '查看', '保存'], workflow: ['拾取颜色。使用 Chrome 扩展直接从当前网页采集颜色。', '查看历史。集中查看最近 20 个颜色，并复制需要的格式。', '保存到项目。将有用的颜色放入可复用的项目调色板中。'], free: ['从网页拾取颜色', 'HEX、RGB 和 HSL 值', '20 色历史记录', '单个项目调色板', 'CSS Variables 导出'], pro: ['免费版全部功能', '提取页面关键颜色', '多个项目调色板', 'Tailwind 配置导出', 'W3C 设计令牌导出'], why: '基础取色器只能回答光标下是什么颜色；TokenTint 还会保留你构建界面时需要的历史颜色、格式和项目上下文。', related: [{ href: '/website-color-picker', label: '从网站创建调色板' }, { href: '/css-variables-generator', label: '将拾取颜色导出为 CSS Variables' }, { href: '/tools', label: '探索所有颜色工作流' }, { href: '/support', label: '获取设置帮助' }], faqs: [{ question: '什么是 Chrome color picker 或 eyedropper 扩展？', answer: '它是一款可以直接从网页采集颜色并复制颜色值的浏览器扩展。TokenTint 支持 HEX、RGB 和 HSL。' }, { question: '如何在 Chrome 中拾取网页的 HEX 颜色？', answer: '在当前网页打开 TokenTint，启动 eyedropper，选择需要的颜色并复制 HEX 值；也可以将同一颜色切换为 RGB 或 HSL。' }, { question: 'Chrome 取色器会保存颜色历史吗？', answer: '会。免费工作流保留最近 20 个拾取的颜色，方便你在当前任务中查看和复用。' }, { question: '可以将拾取的颜色保存到项目吗？', answer: '可以。你可以将历史记录中的颜色加入项目调色板，并将调色板导出为 CSS Variables。' }, { question: '从网页取色需要上传截图吗？', answer: '不需要。TokenTint 通过用户主动启动的 Chrome 扩展工作流，从正在浏览的网页取色。' }], valueCards: [{ title: '直接在浏览器中拾取', description: '无需离开正在研究或构建的网页即可采集颜色。' }, { title: '保留颜色上下文', description: '查看最近颜色，并保留页面或组件周围的小型调色板。' }, { title: '复制正确格式', description: '在将颜色放入项目之前，在 HEX、RGB 和 HSL 之间切换。' }] };
  }
  if (props.h1.includes('Design Token')) {
    return { ...props, ...common, h1: '从网站颜色生成 Design Tokens', intro: '将真实网站和界面中采集的颜色转换为可复用的 CSS 或 W3C 颜色令牌，而不是试图替代完整的设计系统管理平台。', problem: '当颜色从浏览器复制到表格、之后再进行命名时，设计令牌很容易变得繁琐。这段手工流程会造成值不一致，也会拖慢设计与代码之间的反馈。', solution: 'TokenTint 提供一个专注的空间来收集颜色、将颜色归入项目调色板，并导出 CSS Variables 或 W3C Design Tokens。命名、语义决策和后续系统工作仍由你现有的工具和工作流负责。', workflow: ['从实时界面采集颜色，或直接使用已有颜色值。', '检查调色板，并将相关颜色归入同一个项目。', '导出适合开发的格式，再在代码库中完善名称和语义。'], free: ['颜色采集与格式转换', '20 个颜色历史记录', '单个项目调色板', 'CSS Variables 导出'], pro: ['多个项目调色板', 'W3C Design Tokens 导出', '页面颜色提取', 'Tailwind 配置导出', '包含免费版全部功能'], why: '它把设计系统的起点——界面中的真实颜色——连接到团队可以带入代码的导出格式，而不是一个孤立的转换计算器。', related: [{ href: '/color-picker-chrome-extension', label: '从 Chrome 取色器开始' }, { href: '/website-color-picker', label: '采集参考页面调色板' }, { href: '/css-variables-generator', label: '导出 CSS 自定义属性' }, { href: '/tailwind-color-generator', label: '导出 Tailwind 颜色' }], faqs: [{ question: '如何把网站颜色转换成 design tokens？', answer: '从当前网站采集颜色，将需要的值整理到 TokenTint 项目调色板，然后导出为 CSS Variables 或 W3C Design Tokens。' }, { question: 'TokenTint 支持 W3C Design Tokens 格式吗？', answer: '支持。W3C Design Tokens 导出属于 Pro 功能；免费版可以将项目颜色导出为 CSS Variables。' }, { question: 'TokenTint 生成哪些设计令牌？', answer: 'TokenTint 专注于从真实界面采集的颜色令牌，不生成排版、间距、动效或整个设计系统。' }, { question: '这是完整的设计令牌管理平台吗？', answer: '不是。TokenTint 专注于颜色采集、分组和导出，命名规范、语义角色和文档仍由现有工作流负责。' }, { question: '可以从实时网站生成令牌吗？', answer: '可以。你可以从当前网页拾取颜色，或使用 Pro 页面提取功能创建更广泛的起始调色板，再执行导出。' }] };
  }
  if (props.h1.includes('CSS Variables')) {
    return { ...props, ...common, h1: '从网页颜色生成 CSS Variables', intro: '从真实网页采集颜色，整理成项目调色板，再免费导出可复用的 CSS 自定义属性，无需逐个重新输入 HEX 值。', problem: '把浏览器中的颜色逐个复制到样式表，会切断参考页面与代码之间的联系。颜色值容易散落在笔记、重复声明和不一致的变量名称中。', solution: 'TokenTint 将采集、整理和导出放在同一个工作流中：从网页取色、集中检查调色板，并免费导出 CSS Variables。导出结果是一个清晰起点，你可以继续按照组件或设计系统规范调整名称。', workflow: ['采集。从当前网页拾取需要的品牌色、背景色、文字色、边框色和强调色。', '整理。集中检查颜色值，并将需要的颜色保存到一个项目调色板。', '导出。下载 CSS Variables，再在代码库中完善变量名称和语义角色。'], workflowKickers: ['采集', '整理', '导出'], free: ['从网页拾取颜色', 'HEX、RGB 和 HSL 格式', '20 色历史记录', '单个项目调色板', 'CSS Variables 导出'], pro: ['包含免费版全部功能', '提取页面关键颜色', '多个项目调色板', 'Tailwind 配置导出', 'W3C Design Tokens 导出'], why: '它将颜色来源、选定的调色板和 CSS 输出连接起来。基础转换器只改变格式，而 TokenTint 保留实现之前完整的颜色采集流程。', related: [{ href: '/website-color-picker', label: '从网站提取颜色' }, { href: '/design-token-generator', label: '将颜色转换为设计令牌' }, { href: '/tailwind-color-generator', label: '导出 Tailwind-ready 调色板' }], faqs: [{ question: '如何根据网站颜色创建 CSS Variables？', answer: '在 Chrome 中打开网站，使用 TokenTint 拾取需要的颜色，将它们保存到项目调色板，然后导出为 CSS Variables。' }, { question: 'CSS Variables 导出是免费的吗？', answer: '是的。免费版包含一个项目调色板和 CSS Variables 导出。Pro 额外提供页面颜色提取、多个项目、Tailwind Config 和 W3C Design Tokens。' }, { question: 'TokenTint 会自动决定语义变量名称吗？', answer: '导出会提供实用的起始结构。你仍可以根据代码库规范决定 brand、surface、text 和 border 等名称。' }, { question: '保存的颜色会上传吗？', answer: '不会。颜色和项目保存在 Chrome 本地存储中，不会同步到 TokenTint 账户。' }] };
  }
  if (props.h1.includes('Tailwind')) {
    return { ...props, ...common, h1: '从网站颜色生成 Tailwind-ready 调色板', intro: '将参考网站或产品界面中发现的颜色整理为 Tailwind-ready 调色板，并导出 Tailwind Config 起点，无需重新输入每个值。', problem: 'Tailwind 项目需要一致的命名颜色，但源颜色往往存在于浏览器原型或现有网站中。基础取色器只能复制 HEX，无法帮助你收集、比较并带入配置。', solution: 'TokenTint 让你先采集并整理调色板，之后通过 Pro 导出 Tailwind 配置片段，并与其他项目格式一起使用。', workflow: ['从正在研究的界面采集品牌色、背景色、文字色和强调色。', '将颜色收集到 TokenTint 项目调色板中，并确认各种格式。', '使用 Pro Tailwind 导出作为配置起点，再补充符合代码库约定的语义名称和色阶。'], free: ['EyeDropper 取色', 'HEX、RGB 和 HSL 格式', '20 个颜色历史记录', '单个项目调色板', 'CSS Variables 导出'], pro: ['Tailwind 配置导出', '页面颜色提取', '多个项目调色板', 'W3C Design Tokens 导出', '包含免费版全部功能'], why: '它保留了颜色来源的上下文，并为 Tailwind 用户提供从浏览器参考到实现的直接桥梁，而不是留下一堆没有名称的 HEX 值。', related: [{ href: '/website-color-picker', label: '从网站拾取颜色' }, { href: '/css-variables-generator', label: '导出 CSS 自定义属性' }, { href: '/design-token-generator', label: '生成设计令牌' }, { href: '/tools', label: '探索所有颜色工作流' }], faqs: [{ question: '如何将网站颜色导出到 Tailwind？', answer: '从当前网页采集颜色，在 TokenTint 项目中整理需要的值，然后使用 Pro 导出 Tailwind Config 起点。' }, { question: '可以根据现有网站调色板创建 Tailwind config 吗？', answer: '可以。TokenTint 将你采集的颜色导出为适合配置的颜色对象，你可以继续在代码库中完善名称、色阶和语义角色。' }, { question: 'TokenTint 会生成 50–950 色阶吗？', answer: '不会。导出反映你实际采集的颜色，色阶和语义名称仍由你根据项目需要决定。' }, { question: 'Tailwind Config 导出是免费的吗？', answer: 'Tailwind Config 导出属于 Pro 功能。免费用户仍可取色、整理一个项目调色板并导出 CSS Variables。' }] };
  }
...
function alignChineseWorkflow(copy: SeoLandingPageProps): SeoLandingPageProps {
  if (copy.h1.includes('CSS Variables')) return copy;
...
export default function SeoLandingPage(props: SeoLandingPageProps) {
  ...
  const visual = locale === 'zh-CN'
    ? { ...(copy.visual ?? getPageVisual(copy.h1)), heroAlt: copy.h1.includes('Tailwind') ? 'TokenTint 扩展中的 Tailwind 配置导出' : copy.h1.includes('Design Token') ? 'TokenTint 扩展中的项目令牌' : copy.h1.includes('CSS Variables') ? 'TokenTint 项目调色板和 CSS Variables 导出' : copy.h1.includes('Website') ? 'TokenTint 扩展中的页面颜色提取' : 'TokenTint 扩展中的取色功能', showcaseTitle: copy.h1.includes('Tailwind') ? '更快生成 Tailwind 调色板' : copy.h1.includes('Design Token') ? '从采集颜色到可用令牌' : copy.h1.includes('CSS Variables') ? '从网页颜色到可用 CSS' : copy.h1.includes('Website') ? '看见界面背后的颜色' : '从点击取色到交付代码' }
    : (copy.visual ?? getPageVisual(copy.h1));
  const landingPath = props.h1.includes('Chrome Color') ? '/color-picker-chrome-extension' : props.h1.includes('Website Color') ? '/website-color-picker' : props.h1.includes('CSS Variables') ? '/css-variables-generator' : props.h1.includes('Design Token') ? '/design-token-generator' : '/tailwind-color-generator';
  const webPageJsonLd = {
  ...
  const translatedOutputExample = copy.outputExample && locale === 'zh-CN' ? { ...copy.outputExample, label: props.h1.includes('Tailwind') ? 'TAILWIND 配置' : props.h1.includes('CSS Variables') ? 'CSS 自定义属性' : '实际导出', title: props.h1.includes('Tailwind') ? '导出你收集的调色板，而不是虚构的色阶。' : props.h1.includes('CSS Variables') ? '导出样式表可以直接使用的调色板。' : '在导出前查看颜色令牌结构。', description: '先生成适合代码库使用的起点，再根据项目需要完善名称和语义。', footer: props.h1.includes('Tailwind') ? 'Pro：Tailwind 配置导出' : props.h1.includes('CSS Variables') ? '免费版：CSS Variables 导出' : 'Pro：W3C 设计令牌导出' } : copy.outputExample;
  const faqJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',

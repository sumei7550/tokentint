...
function WorkflowSection({ kind, data }: { kind: 'free' | 'pro'; data: WorkflowData }) {
  ...
  return <section className={`workflow-section workflow-${kind}`} id={kind === 'free' ? 'free-workflow' : 'pro-workflow'}><div className="container"><div className="workflow-heading"><p className="eyebrow">{data.eyebrow}</p><h2>{data.title}</h2><p>{data.description}</p>{offer && <p className="workflow-offer">{offer}</p>}</div><div className="workflow-grid">{data.steps.map(([number, title, description, image, alt]: Step) => { const [width, height] = imageDimensions[image]; return <article className="workflow-card" key={title}><div className="workflow-card-copy"><span className="workflow-number">{number}</span><h3>{title}</h3><p>{description}</p></div><div className="workflow-card-media"><img src={image} alt={alt} width={width} height={height} /></div></article>; })}</div></div></section>;
}
...
export default function Home() {
  ...
  return <><Navigation /><main><section className="hero"><div className="container hero-grid"><div className="hero-copy"><p className="hero-kicker">{copy.hero.kicker}</p><h1><span>{copy.hero.title[0]}</span><span className="hero-accent">{copy.hero.title[1]}</span></h1><p className="hero-sub">{copy.hero.subtitle}</p><div className="hero-ctas"><Link href={chromeStoreUrl} className="cta-button" data-analytics-location="hero">{copy.hero.primary}</Link><a href="#free-workflow" className="cta-secondary">{copy.hero.secondary}</a></div><p className="hero-note"><span>●</span>{copy.hero.note}</p></div><div className="hero-product-shot"><img src="/images/screenshots/pick-color-free.png" alt={copy.hero.alt} width="706" height="602" fetchPriority="high" /></div></div></section><WorkflowSection kind="free" data={copy.free} /><WorkflowSection kind="pro" data={copy.pro} /><ExportExample locale={locale} /><section className="pricing"><div className="container"><div className="section-heading"><p className="eyebrow">{locale === 'zh-CN' ? '按你的工作方式选择' : 'CHOOSE YOUR WORKFLOW'}</p><h2>{copy.pricing.title}</h2></div><div className="pricing-cards"><PricingCard title={copy.pricing.free.title} price="$0" features={copy.pricing.free.features} cta={copy.pricing.free.cta} href={chromeStoreUrl} headingLevel="h3" /><PricingCard title={copy.pricing.pro.title} price="$15" billing={copy.pricing.pro.billing} features={copy.pricing.pro.features} cta={copy.pricing.pro.cta} href={localizedPath('/upgrade', locale)} featured headingLevel="h3" /></div></div></section></main><Footer /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(softwareApplicationJsonLd) }} /></>;
}
